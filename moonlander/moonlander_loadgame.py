...
def mainGamePlayer(nr_arquivo):
    try:
        # Initialize pygame and etc.
        screen, font, game, fpsClock = initialize()
        if nr_arquivo < 10:
            ds_arquivo = "moonlander-20221129-0"+str(nr_arquivo)+".pickle"
        else:
            ds_arquivo = "moonlander-20221129-"+str(nr_arquivo)+".pickle"
        gameLoad = loadGame(ds_arquivo)
        
        i = 0
        # Main game loop
        while game.alive:
            # Copy current game state and add the new state for modifications
            gs = copy.copy(gameLoad.states[i])
            game.states += [gs]
